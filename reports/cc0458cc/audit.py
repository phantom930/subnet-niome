"""Replay cc0458cc uploads; distinguish h4's fallback and later unsubmitted rebuilds."""
import hashlib
...
TASK_ID = "cc0458cc-f62c-43db-946e-a7d835e5bf69"
...
def main():
    ...
    predicted = list(range(200, 300)) + list(range(400, 500)) + list(range(900, 1000))
    ...
    result = {
        ...
        "predicted_windows": [200, 400, 900], "predicted_joined": predicted,
        "complement_joined": complement, "plan_generated_at": "2026-09-18T22:17:11.715495+00:00",
        ...
        "definitions": {
            ...
            "candidate_window": "Historical 150-seed circular slice of predicted 300 seeds (h0–h5, stride 50) or complementary 600 seeds (h6–h9, stride 150).",
        },
        "hotkeys": {},
    }
    for i in range(10):
        ...
        joined = predicted if i < 6 else complement
        offset = i * 50 if i < 6 else (i - 6) * 150
        group = "predicted" if i < 6 else "complement"
        candidates = [joined[(offset + j) % len(joined)] for j in range(150)]
        events = ev["events"]
        original_events = [e for e in events if "2026-09-18 22:19" <= e[:16] < "2026-09-18 22:30"]
        later_events = [e for e in events if "2026-09-18 23:07" <= e[:16]]
        window_lines = [e for e in events if "Build: window " in e]
        assert window_lines and all("200-299,400-499,900-999" in e and "2026-09-18T22:17" in e for e in window_lines)
        candidate_lines = [e for e in original_events if "Build: band candidates " in e]
        assert candidate_lines and all(f"(150 seeds, {group} space of {len(joined)}, offset {offset}) | cut 900 seeds" in e for e in candidate_lines)
        if i != 4:
            assert any("using the prepared submission (250 rows" in e for e in events)
        else:
            assert any("hedges skipped (a prepared build is still holding the GPU); using the ordinary construction" in e for e in original_events)
            assert not any("using the prepared submission" in e for e in events)
        ...
        if i != 4:
            ...
            assert logged == (11, 11, 100, 137, 900)
            assert cas_mix == {"Cas12a": 76, "Cas9": 174}
            assert uploaded_events[0][:23] < builds[0][:23]
        later_builds = [e for e in later_events if "Build: conjunction " in e]
        assert len(later_builds) == 1
        later_logged = tuple(map(int, re.search(r"band (\d+) k=(\d+) group (\d+) clean (\d+)/(\d+)", later_builds[0]).groups()))
        assert later_logged[:3] == (12, 12, 100) and later_logged[4] == 900
        record_band = sorted(json.loads(ev["current_window_record"]["band"]))
        assert len(record_band) == 12 and record_band != band
        assert ev["current_window_record"]["at"].replace("T", " ")[:19] > uploaded_events[0][:19]
        rec = {
            ...
            "candidate_window_applies_to_upload": i != 4,
            "cut_search_space": list(range(100, 1000)) if i != 4 else None,
            ...
            "later_logged_clean_count": later_logged[3], "original_logged_clean_count": logged[3],
