"""Replay nine conjunctions and one all-HDR upload for task 35e41a29."""
import hashlib
...
def main():
    ...
    conjunction_space = list(range(500, 600))
    hdr_space = list(range(100, 200))
    conjunction_cut = sorted(conjunction_space + hdr_space)
    result = {
        ...
        "definitions": {
            ...
            "candidate_window": "h0–h8: width-12 windows at stride 12 over 500–599; h8 wraps to 500–507 ∪ 596–599. Shared cut 100–199 ∪ 500–599. h9: all-HDR search across 100–199.",
            ...
            "all_hdr_log_clean": "For h9, log clean 7/100 is the Cas12a all-HDR intersection in the 100-seed candidate window, not the full-row no-cut-free set. Cas9 rows are then selected to return HDR on those seven seeds.",
        },
        "hotkeys": {},
    }
    for i in range(10):
        ...
        offset, width = (12 * i, 12) if is_conjunction else (0, 100)
        ...
        if is_conjunction:
            ...
            assert f"12 seeds (shared window, loop {loop_label} of 9) | cut 200 seeds (narrow) | k=8" in candidate_lines[0]
        else:
        ...
        if is_conjunction:
            ...
            assert logged == (8, 8, 80, len(ca_clean_in_cut), 200)
            ...
            assert logged == (80, len(set(ca_hdr) & set(candidates)), 100)
        ...
        assert len(band) == (8 if is_conjunction else 7) and set(band) <= set(candidates)
        ...
        rec = {
            ...
            "band_group": "conjunction_12_seed_window" if is_conjunction else "all_hdr_100_seed_window", "band_space": joined, "band_space_ranges": ranges(joined),
            "candidate_offset": offset, "band_loop": 1 if is_conjunction else None, "logged_band_loop": 1 if i < 7 else None,
