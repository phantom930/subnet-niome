"""Replay six narrow-cut conjunctions and h6's fallback; h1–h3 have no build/upload."""
import hashlib
...
TASK_ID = "31138352-9ccf-4fa0-907e-dd010395e27a"
...
def main():
    ...
    result = {
        ...
        "complement_joined": complement, "plan_generated_at": "2026-09-20T12:24:45.401309+00:00",
        ...
        "definitions": {
            ...
            "candidate_window": "Historical 300-seed slice of full 100–999 at offsets 0/100/200/300/400/500/600 for h0/h4/h5/h6/h7/h8/h9. Also the conjunction cut-search window; h6 uploaded a fallback instead.",
    ...
    for i in range(10):
        ...
        assert window_lines and all("100-399" in e and "2026-09-20T12:24" in e for e in window_lines)
        candidate_lines = [e for e in events if "Build: band candidates " in e]
        assert candidate_lines and all(f"(300 seeds, predicted space of 900, offset {offset}) | cut 300 seeds (union) | k=8" in e for e in candidate_lines)
        if h != "h6":
        ...
        if h != "h6":
            assert logged == (8, 8, 80, len(set(ca_clean) & set(candidates)), 300)
            assert set(clean) & set(candidates) == set(ca_clean) & set(candidates)
            assert len(band) == 8 and set(band) <= set(candidates)
            assert cas_mix == {"Cas12a": 80, "Cas9": 170}
        else:
            assert logged == (8, 8, 80, 26, 300)
        ...
        assert len(record_band) == 8
        assert (record_band == band) == (h != "h6")
        rec = {
            ...
            "candidate_window_applies_to_upload": h != "h6",
            "cut_search_space": candidates if h != "h6" else None,
