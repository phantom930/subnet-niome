"""Replay seven uploaded conjunctions; keep unavailable scoring seeds distinct from probes."""
import hashlib
...
TASK_ID = "5a453e21-5516-415c-a2f9-2ad2d04a75e2"
...
def main():
    ...
    seeds = [int(x) for x in str(stamp).split(",")] if str(stamp) not in ("0", "", "None") else None
    validation_seeds = seeds or [manifest["diagnostic_seed"]]
    ...
    joined = list(range(200, 300))
    cut = joined + list(range(400, 500))
    result = {
        ...
        "diagnostic_seed": manifest["diagnostic_seed"] if seeds is None else None,
        ...
        "band_joined": joined, "shared_cut_joined": cut, "layout_source": "hdr_fixed_layout",
        ...
        "definitions": {
            ...
            "candidate_window": "Seven width-15 windows at stride 15 around 200–299. h6 wraps to 200–204 ∪ 290–299. Every hotkey uses loop 1 and shared cut 200–299 ∪ 400–499.",
    ...
    for i in range(7):
        ...
        candidates = sorted({joined[(15 * i + j) % len(joined)] for j in range(15)})
        events = ev["events"]
        ...
        rec = {
            ...
            "band_group": "fixed_15_seed_window", "band_space": joined, "band_space_ranges": ranges(joined),
            "candidate_offset": 15 * i, "band_loop": 1,
    ...
    assert set(result["fleet_band_union"]) <= set(joined)
    (OUT / "analysis.json").write_text(json.dumps(result, indent=2) + "\n")
