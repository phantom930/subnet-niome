"""Replay seven uploads for af6b11ee; keep h1–h3's prepared-only records separate."""
import hashlib
...
TASK_ID = "af6b11ee-dbc2-45b5-86cb-9ce888dba0c3"
...
def main():
    ...
    result = {
        ...
        "complement_joined": complement, "plan_generated_at": "2026-09-20T03:17:17.030525+00:00",
        ...
        "definitions": {
            ...
            "candidate_window": "Historical 30-seed slice of predicted 100–399, at offset 30*i for h0–h9; confirmed by original build logs.",
            "missing_upload": "h1–h3 have prepared builds but no received-task/upload events, no upload archives and no official score entries. Submitted sets and ranks are unavailable, not zero.",
        },
        "hotkeys": {},
    }
    for i in range(10):
        ...
        offset = i * 30
        group = "predicted"
        candidates = joined[offset:offset + 30]
        events = ev["events"]
        window_lines = [e for e in events if "Build: window " in e]
        assert window_lines and all("100-399" in e and "2026-09-20T03:17" in e for e in window_lines)
        candidate_lines = [e for e in events if "Build: band candidates " in e]
        assert candidate_lines and all(f"(30 seeds, {group} space of {len(joined)}, offset {offset}) | cut 900 seeds (wide) | k=11" in e for e in candidate_lines)
        ...
        assert len(record_band) == 11 and set(record_band) <= set(candidates)
        if not entry["archive"]:
            assert score is None
            assert not any("Received genomics task" in e or "Submitted 250 rows" in e for e in events)
            assert logged[:3] == (11, 11, 100) and logged[4] == 900
            result["hotkeys"][h] = {
                "hotkey": entry["hotkey"], "uid": None, "construction": "prepared_only",
                ...
                "clean_hits": None, "band_hits": None, "logged_prepared_clean_count": logged[3],
                "prepared_band_record": record_band, "prepared_band_scoring_seed_overlap": sorted(set(record_band) & set(seeds)),
                "latest_record_at": ev["current_window_record"]["at"],
                "status": "No uploaded archive, received-task/upload log event or official score entry. Prepared counts/bands are not submitted results.",
            }
            print(h, "prepared only; no upload archive or score entry", flush=True)
            continue
        ...
        rec = {
            ...
            "band_group": group, "band_space": joined, "band_space_ranges": ranges(joined),
            "candidate_offset": offset, "candidate_window": candidates, "candidate_window_ranges": ranges(candidates),
            "cut_search_space": list(range(100, 1000)),
