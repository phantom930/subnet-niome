"""Audit the uploaded h0-h5 archives for task 4f3cc5a5, without changing miner data.

Run from the repository root with .venv/bin/python reports/4f3cc5a5/audit.py.
...
TASK_ID = "4f3cc5a5-cbb5-4a1e-86c4-2c88d2e51697"
HOTKEYS = [
...
def main():
    ...
    result = {
        ...
        "joined_band_space": joined, "cut_space": joined, "scan_space": [100, 999],
        "definitions": {
            ...
            "cas12a_clean_seeds": "All 80 submitted Cas12a rows cut over 100-999; the subset in the joined cut window is checked against the build log.",
            "clean_seeds_in_cut_space": "Full-row clean seeds inside the 300-seed joined cut window.",
            "clean_seeds_outside_cut_space": "Any additional full-row clean seeds outside the cut search window, measured by replay.",
    ...
    for i, hk in enumerate(HOTKEYS):
        ...
        clean_in_cut = sorted(set(clean) & set(joined))
        ca_clean_in_cut = sorted(set(ca_clean) & set(joined))
        assert tuple(map(int, match.groups())) == (8, 8, 80, len(ca_clean_in_cut), 300)
        assert clean_in_cut == ca_clean_in_cut
        assert Counter(r["cas_system"] for r in rows) == {"Cas12a": 80, "Cas9": 170}
        rec = {
            ...
            "clean_seeds_outside_cut_space": sorted(set(clean) - set(joined)),
            "cas12a_clean_seeds_outside_cut_space": sorted(set(ca_clean) - set(joined)),
