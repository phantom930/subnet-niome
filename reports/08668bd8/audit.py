"""Replay 08668bd8 uploads, keeping h4's fallback separate from its prepared band."""
import hashlib
...
TASK_ID = "08668bd8-9461-41b1-b1e4-083dec3c9f18"
...
def main():
    ...
    result = {
        ...
        "complement_joined": complement, "plan_generated_at": "2026-09-19T00:17:09.880547+00:00",
    ...
    for i in range(10):
        ...
        assert window_lines and all("100-399" in e and "2026-09-19T00:17" in e for e in window_lines)
        candidate_lines = [e for e in events if "Build: band candidates " in e]
        assert candidate_lines and all(f"(150 seeds, {group} space of {len(joined)}, offset {offset}) | cut 900 seeds (wide) | k=12" in e for e in candidate_lines)
        if i != 4:
            assert any("using the prepared submission (250 rows" in e for e in events)
        else:
            assert any("hedges skipped (a prepared build is still holding the GPU); using the ordinary construction" in e for e in events)
        ...
        if i != 4:
            assert logged == (12, 12, 100, len(ca_clean), 900)
            assert clean == ca_clean
            assert len(band) == 12 and set(band) <= set(candidates)
            assert cas_mix == {"Cas12a": 100, "Cas9": 150}
        else:
            assert logged == (12, 12, 100, 94, 900)
            assert cas_mix == {"Cas12a": 75, "Cas9": 175}
            assert uploaded_events[0][:23] < builds[0][:23]
        record_band = sorted(json.loads(ev["current_window_record"]["band"]))
        assert len(record_band) == 12
        assert (record_band == band) == (i != 4)
        rec = {
