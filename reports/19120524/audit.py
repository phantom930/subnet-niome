"""Replay the six actual uploads for 19120524; keep four unsubmitted prepared builds separate."""
import hashlib
...
TASK_ID = "19120524-b0f1-4061-b99b-92e4599e98d3"
...
def main():
    ...
    result = {
        ...
        "definitions": {"clean_seeds": "All 250 uploaded rows cut at this seed, replayed over 100-999.",
                        ...
                        "candidate_window": "Historical 150-seed circular slice of 100-399 at offset 30*i for h0-h9, confirmed by original stride-30 plan logs and replay.",
                        "missing_archive": "h0/h4/h5/h6 have no task upload archives and zero valid experiments in the API. Actual submitted sets cannot be reconstructed."},
        "hotkeys": {},
    }
    for i in range(10):
        h = f"h{i}"
        ...
        score = best[hk]
        joined = list(range(100, 400))
        offset = i * 30
        candidates = [joined[(offset + j) % 300] for j in range(150)]
        original_events = [e for e in ev["events"] if "2026-09-18 17:30" <= e[:19] < "2026-09-18 18:40"]
        window_lines = [e for e in original_events if "Build: window " in e]
        assert window_lines
        expected_window = "100-399"
        ...
        rec = {
            ...
            "joined_source": "plan (16:40 plan; windows 100/200/300)",
            "candidate_offset": offset, "candidate_window": candidates,
            "candidate_window_basis": "Original ten-hotkey stride-30 layout from the historical plan logs, checked against uploaded bands.",
            "candidate_window_ranges": ranges(candidates), "candidate_window_applies_to_upload": bool(entry["archive"]),
        ...
        if not entry["archive"]:
            ...
            assert prepared
            logged_counts = [int(re.search(r"clean (\d+)/900", e)[1]) for e in prepared]
            assert len(set(logged_counts)) == 1
            rec.update({"rows": None, "clean_seeds": None, "band_seeds": None, "clean_hits": None,
                        "band_hits": None, "submitted": None, "logged_prepared_clean_count": logged_counts[-1],
        ...
        assert construction == "conjunction"
        builds = [e for e in original_events if "Build: conjunction " in e]
        assert builds
        for build in builds:
            logged = tuple(map(int, re.search(r"band (\d+) k=(\d+) group (\d+) clean (\d+)/(\d+)", build).groups()))
