...
* **This is a PER-HOTKEY measurement and the fleet question is open.** Correlated siblings are what
  ...
  **If the fleet regrows, rotate `ConjunctionConfig.band_offset_frac` per hotkey and re-price with
  [fleet_price.py](fleet_price.py) before trusting any of the above.** **2026-09-21: the fleet HAS
  regrown — ten band hotkeys — and only the first half of that instruction has been carried out.**
  The offsets do rotate per hotkey (tier A at stride 100 over 900, tier C at stride 100 over the
  predicted 300) and the bands are verified disjoint WITHIN tier C on a live round, but the
  **re-pricing with `fleet_price.py` has NOT been done**, so every ratio in the table above is still
  a per-hotkey number standing in for a fleet of ten. Tier A's windows overlap by up to 200 of 300,
  so its siblings are the ones most likely to correlate and the ones the re-pricing is for.
...
and wrong about the risk.** **SUPERSEDED 2026-09-21: `WIDE_CUT_CELLS` is EMPTY — every cell is on
`narrow` now. The wall/pool/margin arithmetic below is still the right reading of the wide arm, and
the wide arm is no longer shipped anywhere.** Three fresh HEK293 contracts, both arms at the LIVE config (`band_k` 8,
...
**2026-09-18 (shipped): HEK293 -> `union` cut + k=9, and the erythroid cells -> k=12.**
**NO LONGER SHIPPED — `band_k` went back to 8/11 on 2026-09-19 and the cut to `narrow` on
2026-09-21.** Both halves of the title are stale; the availability evidence below (24 of 24 built at
k=12) stands and is what makes 11 comfortable rather than marginal. Set by
...
**2026-09-21: TEN hotkeys in THREE tiers, h1-h3 restored on the predicted 300 at width 100, and
every cell moved to the NARROW cut.** Set by operator request. This supersedes both the 09-19
stride-30 tiling and the 09-20 seven-hotkey layout below, and it changes the cut span on all four
cells, so read it before any of them.

**h1/h2/h3 are registered again** — uids **189 / 45 / 136**, verified against the chain at block
9113530. Their pm2 apps had been `stopped` (not deleted) since 2026-09-19 11:11, ~41h, while the
plan kept assigning them windows: `DEREGISTERED` listed only `niome_hotkey10`, so three of the ten
30-seed slices were allocated to processes that could not build them. **A stopped hotkey needs
excluding from the allocation for exactly the same reason a deregistered one does**; the plan file
will not tell you, and coverage read off it overstates the fleet by however many are down.

**Every uid in this file older than 2026-09-21 is stale, including ones described as "unchanged".**
h0-h3 all moved between the 09-17 check and this one (122 -> 248, 41 -> 189, 163 -> 45, 118 -> 136)
while h4/h5 kept 190/234. Four of six recycled in four days. Identify our hotkeys by ss58, never by
uid — `fleet_price.py` already does, and this is why.

| tier | hotkeys | space | count x stride | width | overlap |
|---|---|---|---|---|---|
| A (`BAND_HK`) | h0, h4-h9 | the full 100-999 | 6 x 100 + 300 = 900 | 300 | heavy, non-wrapping |
| **C (`PRED_HK`)** | **h1-h3** | **the plan's predicted 300** | **3 x 100 = 300** | **100** | **none — disjoint** |
| B (`REST_HK`) | — | empty | — | — | — |

**h1-h3 could NOT join tier A, and the reason is an import-time assert that would have taken the
whole fleet down.** Tier A's invariant is `(count - 1) * stride + width == span`; at seven that is
6 x 100 + 300 = 900, and at ten it is 9 x 100 + 300 = **1200 against a 900-seed space**. The assert
runs at module import of `joined_window`, so it crash-loops EVERY hotkey, not just the three added.
There is no integer stride that fits ten non-wrapping width-300 windows in 900 either —
(900 - 300)/9 = **66.67**. Tier C exists because of that arithmetic, not by preference.

**Width 100 rather than 300 is the load-bearing choice and `conjunction.py` already names why.** At
`band_width` 300 on a 300-seed space all three hotkeys search the identical candidate set and build
the IDENTICAL band — "the correlation that sank all-cut at fleet level". Tier C's invariant is
therefore the CIRCULAR one (`count * stride == span`, 3 x 100 = 300) with `width == stride`, which
makes the three windows partition their space instead of overlapping across it. Two invariants now
live side by side in `joined_window` and they are not interchangeable: tier A overlaps and must not
wrap, tier C partitions and must.

**`band_width` is per CELL and cannot express this**, so `joined_window.band_sub_width(instance)`
returns the tier's width (300 for A, 100 for C, `None` outside a band group so the cell's own value
survives) and `miner.py` uses it in place of `cj_cfg.band_width`. Anything reading `band_width`
straight off `CELL_CONFIG` now describes tier A only.

**Measured on the live HEK293 round 09b7b842, which is the whole point of the tier:**

| | candidates | band (k=8) | clean | cas9 | cells |
|---|---|---|---|---|---|
| h1 | 100-199 | **8 = k** | 23/300 | 1436 | 8/8 |
| h2 | 200-299 | **8 = k** | 24/300 | 1336 | 8/8 |
| h3 | 300-399 | **8 = k** | 25/300 | 1297 | 8/8 |

The bands were then recovered by replaying stage 3 over each hotkey's SHIPPED 250 rows rather than
inferred from the window config — h1 `[124,127,129,137,140,141,175,185]`, h2
`[203,205,215,228,235,262,272,298]`, h3 `[303,316,325,329,339,345,360,394]`. **Zero pairwise
overlap, union 24 of 24.** That is the disjointness the stride-30 tiling was reaching for, now a
property of the layout rather than a hope. All ten hotkeys built that round and **10 of 10 were
called and submitted** (calls spread 06:19-07:00, a 40-minute window — a partial tally read inside
it looks like failures and is not).

**`CUT_MODE` is now `narrow` on all four cells**, replacing `wide` on the erythroid three and
`union` on HEK293. **"Narrow" means the 300-seed window containing THAT hotkey's own band, not one
global 300** — taking it as "the predicted 300 for everybody" breaks `band ⊆ cut` for tier A, whose
bands come out of the full 900 and mostly sit outside the predicted classes. Per tier: C gets the
predicted 300 (its band space already is that), A gets its own width-300 slice, which is
byte-identical to what `union` returned. Verified `band ⊆ cut` on all ten hotkeys on both K562 and
HEK293 before restarting anything.

**This moves TOWARD what was already measured, and two entries below should be read as settled by
it rather than still open.** `widecut_price.py` priced wide-vs-narrow as a wash on score (+1.1%,
4W/2L/2T, sign p = 0.688) and `coldbuild.py` measured narrow at roughly half the cold build; that
entry's own recommendation was to drop the wide cut on the erythroid cells, recorded as "not done:
the erythroid cells keep the wide cut by operator request". **It is done now.** Confirmed live:
K562 cold build **69s** against the wide cut's measured 410s, HEK293 378s against its 600s gate.
`CONJUNCTION_MIN_BUDGET_S` of 700 on K562/CD34+ was raised FOR the wide cut and is now very
conservative — lowering it would let the rung fire on shorter prefetch leads, and has not been done.

**The standing risk, and the one number to watch.** `CELL_CONFIG`'s k=11 (erythroid) / k=8 (HEK293)
were tuned at `band_width` 300 and are **not re-measured at 100**. `conjunction.py` warns survivors
fall ~0.55x per band seed so a k feasible at 300 is not automatically feasible at 100, and the
falsified `narrow_width` row measured the band GROWING toward the window as the window shrinks,
which makes the Cas9 fill harder rather than easier. Nine builds across two cells all reached
band = k, which is reassuring and is not a measurement. **Watch h1-h3 for
`conjunction declined (band reached N of K)`** — a decline falls through to all-HDR, so it shows up
as quiet quality loss, not an outage.

**2026-09-19: sub-window 30 at stride 30 — ten DISJOINT candidate windows, and `REST_HK` is
empty.** SUPERSEDED by the 2026-09-21 entry above (ten hotkeys in three tiers; h1-h3 left this
tiling when they were stopped and return at width 100 on the predicted 300). Set by operator request, with `band_k` back to 11 (erythroid) / 8 (HEK293), group 100/80
...
**every uid in this sentence except h4's and h5's is now wrong; re-read at block 9113530 they are
248/189/45/136. Four of six recycled in four days — identify by ss58, never by uid** —
...
`WIDE_CUT_CELLS`) — **DONE 2026-09-21, and the predicted saving reproduced: K562 cold build 69s
against the 410s measured here**, which also lets `CONJUNCTION_MIN_BUDGET_S` fall back from 700s to ~450/430s — at
...
**Done 2026-09-21** — every cell is on `narrow`. `CONJUNCTION_MIN_BUDGET_S` has NOT been lowered
with it, so the 700s gates on K562/CD34+ are now far above the ~200s the narrow build costs.
...
| `not_mhnhej` as a SEED-AGNOSTIC (window, not single-seed) hedge, enumerated with no gc_band/max_distance restriction ("all GC, all distance from scratch"), K562 (`sa_nomh_scratch.py`) | **dead, and dramatically worse than the documented repair-mode ceiling of 349-491 failed seeds — not inside it, as the mechanism might have suggested.** The motivating idea was sound on paper: `stage3.microhomology_trigger` gives `p(mh=True) = min(0.6, gc*(1-gc)*2.2)`, which is LOWEST at gc near 0 or 1 and HIGHEST (0.55) at gc=0.5 — the CENTRE of the shipped `SeedAgnosticConfig` band (0.40-0.60) — so pushing gc to either extreme should lower `mh_nhej_w`'s effective weight and hence `P(MH_NHEJ \| cut)`, on top of `hdr_w` already rising with gc-driven energy. Enumerated genuinely from scratch (flank 3000, gc_band 0.0-1.0, no max_distance) on a real K562 contract: the achievable GC range is **0.150-0.700** (real PAM-adjacent guides can't reach the true 0/1 extremes within the mismatch budget), and the per-guide not_mhnhej failure rate across that whole range is **0.191-0.222** — a span of barely 3 percentage points, matching the formula's own near-flat theoretical prediction (0.174-0.181) and confirming the mechanism is real but tiny, exactly as `energy_combo_k562.py`'s AUC ceiling (0.5271) already implied from a different angle. `Pearson(gc, fail_rate)` is only **-0.15**, and the aggregate MINIMUM sits inside the shipped 0.40-0.60 band (0.1932), not at either extreme — "all GC" does not even clearly beat the narrow band it was meant to escape. **The decisive result is the group-size curve, not the per-guide rate**: `min_union_group` (the same greedy the shipped cut-only hedge uses, unmodified) on a 1600-candidate pool collapses explosively as group size grows — union **122/900 (86.4% clean) at group_size 1**, **479/900 (46.8% clean) at 5**, **833/900 (7.4% clean) at 20**, and **886-888/900 (1.3-1.6% clean) at 40-150** — the range any real submission's Cas12a share actually needs. On the IDENTICAL pool, plain cut-only (no repair-mode constraint at all) reaches **532-603/900 (33-41% clean)** at the same group sizes, a huge margin over not_mhnhej at every size tested. **The mechanism is independence, not magnitude**: not_mhnhej's ~20% per-guide failure rate is far higher than cut-only's ~2-6%, AND — critically — it is nearly UNIFORM across the whole design space (the 3-point GC range above), so unlike whatever gives `hdr` its wider, more exploitable spread (349 failed seeds, well below the ~450 a uniform 50% independent-failure model predicts), the min-union greedy has no guide-to-guide heterogeneity to hunt for here: failures are close to independent per (guide, seed) with almost the same rate for every guide, so the union grows close to the `1-(1-p)^N` curve a fully-independent population would produce regardless of pool size or GC/distance breadth. Raising the pool size or widening GC/distance further cannot fix this, for the same reason `CLAUDE.md`'s existing seed-agnostic ceiling note says raising `max_fail`/`max_distance`/pool size "does not move it" for repair-mode rules generally — confirmed here for `not_mhnhej` specifically rather than assumed to be covered by that finding. **Follow-up (2026-09-21, `sa_nomh_w150.py`): narrowing the WINDOW to 150 seeds (100-249) DOES help meaningfully, and an explicit `max_fail` pool cutoff (mirroring `cas12a_max_fail`) mostly does not — and the two results are for different reasons.** Per-guide failure rate is unchanged at width 150 (mean 19.8% of 150, matching the full-window rate — it is a per-seed probability, not a window artefact), but the SMALLER seed universe makes chance coincidence of failures across guides far more likely, exactly the mechanism `conjunction.py`'s own width sweeps exploit for its band: clean fraction at group_size 80 rises from **1.6% (900-wide) to 11-12% (150-wide)**, a ~7x relative improvement, and at group_size 20 from 7.4% to 27%. **It is still nowhere near viable and still dominated by plain cut-only on the identical pool at the identical width**: cut-only reaches 83-91% clean at group_size 40-150 against not_mhnhej's 9-15% — roughly a 7x gap that persists at the narrower width too. The `max_fail` cutoff is nearly inert once set above the achievable floor: `max_fail=10` has **zero survivors** (the best guide in a 4000-candidate pool only reaches 11/150 failures), but every value from 30 up through no-filter-at-all produces IDENTICAL min-union results, because the greedy already gravitates to low-failure guides unaided. Set too tight (max_fail=20, 209 survivors of 4000), it is actively WORSE at larger group sizes than a looser cutoff (group_size 40: 5% clean at max_fail=20 against 15% at max_fail>=30) — restricting the pool that hard denies the greedy access to guides whose failures happen to OVERLAP an already-chosen guide's, which is what min-union actually optimises for, not individual guide quality. So the two levers this follow-up tested (window width, max_fail) behave oppositely: width is the one real lever (window density, not design), and max_fail is inert-to-harmful once past the trivial floor — neither closes the gap to cut-only. |
| exploiting stage4's `KFold(shuffle=True, random_state=seed)` directly — arrange a not_mhnhej submission so all BLUNT-outcome rows land in ONE fold (the other 4 pure HDR), on the theory that "4 folds near-1.0 R2, 1 mixed fold" would average to consistency 0.75-0.95 (`kfold_align_nomh.py`) | **actively HARMFUL, not merely ineffective — consistency_factor came in at 0.0018, far BELOW the ordinary ~0.10-0.30 floor this file documents everywhere, against 0.2743 for an unstructured not_mhnhej build at the SAME pinned seed (`nomh_standalone.py`).** Two preconditions the idea needs were verified true before building anything: `KFold.split()`'s fold membership is a PURE function of `(n, random_state)`, entirely independent of the data passed in (confirmed: identical partitions for two totally different dummy datasets at the same seed); and the miner's submitted row ORDER survives unchanged into stage4's merged DataFrame (confirmed by replaying `flatten_stage3`/`flatten_stage12`/merge on a real 250-row submission — merged order matched submission order 250/250). So a miner who KNOWS the round seed (this only works as a seed_depend-style ORACLE test, see the caveat below) genuinely CAN precompute `KFold(n_splits=5, shuffle=True, random_state=seed).split(range(250))` offline and choose which guide goes at which position. **But the mechanism itself is broken, for a reason found BEFORE running anything and then confirmed exactly**: K-Fold trains each fold's model on the OTHER 4 folds, so the "special" all-BLUNT fold is part of the training set for every one of the 4 "pure HDR" folds' evaluations — there is no way to exclude it from all four at once with only 5 folds total. And `sklearn.metrics.r2_score` on a constant `y_test` is a HARD STEP FUNCTION, not continuous — verified directly: `r2_score([1]*50, [0.99]*50)` returns exactly **0.0**, not something close to 1.0; only an EXACT match returns 1.0. Since no feature combination can perfectly separate HDR from BLUNT within not_mhnhej (the AUC ceiling is 0.5271, `energy_combo_k562.py`), no fold can hit that exact match. **Confirmed on the real pipeline, per-fold, on task 2af71117 seed 401: all 5 folds landed at r2 = +0.0000, including the four "pure" ones.** What makes this WORSE than a simple miss is an apparent overfitting artefact: the deep (`max_depth=12`), 200-tree RandomForest, trained on the 200 rows outside whichever fold is held out, did not predict near the training mean (~0.75) for held-out rows — it predicted with FULL CONFIDENCE and the WRONG SIGN: fold 0 (truly all-BLUNT, y=0) got `mean(pred) = 1.0000` exactly; fold 1 (truly all-HDR, y=1) got `mean(pred) = 0.0000` exactly. `is_hdr`'s MAE came out to **0.9143** — close to the theoretical worst case for a 0/1 target, far worse than a "can't discriminate, predicts near 0.5" outcome would give — which is what drags `avg_nmae` up enough to crater `consistency_factor` to 0.0018. The likely cause: segregating BLUNT rows into a contiguous rank-block per cell (both pools were taken nearest/highest-`weighted_score`-first, same ordering `SD.enumerate_candidates` already sorts by) handed the deep forest a spurious structural correlate of "labeled BLUNT in this dataset" to overfit to, and it generalised that overfit pattern in the wrong direction on held-out rows. **This does NOT transfer to a live, blind round even in principle** — the whole mechanism needs the round seed to already be known at build time to compute the KFold partition and the true HDR/BLUNT labels, which the shipped conjunction (built before broadcast, seeds stamped after) never has; it was only ever testable as an oracle exercise on an already-scored task, exactly like `seed_depend`. **Follow-up (2026-09-21, same script, `KA_SPECIAL_BLUNT=25`): softening the special fold to a genuine 25 HDR / 25 BLUNT mix (instead of all-50 BLUNT) is LESS harmful but still falls well short of both the predicted range and the naive baseline.** `consistency_factor` rose to **0.1468** (from 0.0018) but stayed below the unstructured build's 0.2743. Per-fold replay explains why: the special fold's `y_test` is genuinely non-constant now (mean 0.500), so its own r2 is computed normally rather than snapped by the degenerate rule — but the model still predicted it with full confidence in one direction (`mean(pred)=1.0000` exactly on a truly 50/50 fold, r2=-0.7091). The four "pure HDR" folds still ALL land at r2=+0.0000 — none hits the exact match `r2_score` requires — but the predictions are less extremely wrong than the all-BLUNT variant (0.37-0.70 rather than snapping to exactly 0.0000), because a genuinely mixed special fold gives the deep forest less of a clean spurious pattern to overfit to, so `is_hdr`'s MAE recovers to 0.4837 (from 0.9143). Softening the split reduces the self-inflicted damage but cannot create the exact-match condition no fold combination reaches; the predicted 0.75-0.95 range was not approached by either variant. **Follow-up (2026-09-21, `mixrate_nomh.py`): dropping the KFold-alignment idea entirely and just sweeping the plain HDR:BLUNT ratio (UNSTRUCTURED row order, no fold targeting) finds the true best non-trivial mix, and it is NOT a "moderate" ratio — it is the smallest possible contamination.** Same task/seed (2af71117, seed 401). blunt=1 of 250 (249 HDR / 1 BLUNT) gives `consistency_factor` **~0.4999**, roughly half the trivial all-HDR ceiling (1.0000) and by far the best non-trivial point measured. From there the curve falls steeply and MONOTONICALLY: blunt=2 → 0.4801, blunt=3 → 0.4577, blunt=4 → 0.3946, blunt=5 → 0.3404 — then past ~6 rows it stops falling and flattens into a noisy plateau, **roughly 0.22-0.37 all the way out to blunt=125 (50%)**, with no further systematic decline (a coarser sweep at 0/2/5/10/15/20/25/30/40/50/62/75/91/100/125 confirms the plateau; one outlier crash to 0.2283 at blunt=8 reproduces the same confidently-wrong-prediction failure mode `kfold_align_nomh.py` found more severely when BLUNT rows were deliberately segregated, here occurring by chance at low n). Mechanism: `is_hdr`'s R2 is negative at every non-trivial point tested and therefore contributes exactly zero to `consistency_score` regardless of its magnitude (`max(avg_r2, 0)`), so `consistency_score` is driven almost entirely by `is_hdr`'s normalized MAE, which scales with BLUNT count as expected for a model still close to "always predict HDR" while BLUNT is rare but with nothing to exploit once BLUNT is common. **Practical upshot: there is no sweet-spot ratio to search for — minimising BLUNT count is the only lever, and once contamination passes a handful of rows the exact ratio barely matters.** This supersedes the framing of "find the best mix rate" with "there is no best mix rate other than the minimum achievable one," and is a cleaner, non-adversarial confirmation of the same ceiling `kfold_align_nomh.py` hit from the structured-alignment angle: not_mhnhej cannot get anywhere near `hdr`'s exact-1.0 by ANY row arrangement once even one row disagrees. |
...
| energy as a proxy to distinguish HDR from BLUNT_NHEJ inside `not_mhnhej`, on K562 and HEK293; extended to a 5-factor combination (consistency, gc, dist, energy, mh); extended again to the REVERSED hypothesis (low energy = HDR) on `not_mhnhej` run as its OWN standalone construction, no all-cut, on K562 (`energy_hdr_k562.py`, `energy_combo_k562.py`, `nomh_standalone.py`) | **the reversed-sign test cannot even be run, for the same structural reason as the original: the REAL allocator saturates energy to exactly 1.0000 regardless of which sign is under test.** `nomh_standalone.py` calls `seed_depend`'s actual allocator (`SD.allocate`, what `SD.build()` calls) with `cfg.rule="not_mhnhej"` and nothing else — no cut-min-union, no band, no conjunction, i.e. the literal standalone construction the user asked to test — rather than the hand-picked nearest-first/spread/uniform-high samples the earlier scripts used. On a real K562 contract (task 2af71117, real seeds 401/403/249, built pinned to seed 401): the allocator's own weighted_score ranking (nearest, highest-GC guides first) drives energy to **mean 1.0000, std 0.0000, min 1.0000, max 1.0000** across all 250 real allocated rows — the same saturation the hand-picked `default`/`uniform_high` arms hit, now confirmed on the actual objective-maximising construction rather than a convenience sample. `Pearson(energy, is_hdr) = +0.0000` follows by mathematical necessity (zero variance in one variable makes any correlation, either sign, undefined/zero), so **the hypothesis "low energy = HDR, high energy = BLUNT" is not falsified so much as unaskable on this build** — there is no energy variation for either sign to ride on. Tercile means (0.5542 / 0.7349 / 0.6190 on P(HDR), all at mean energy exactly 1.0000) are pure sampling noise around the same saturated point, not a trend in either direction. **The standalone construction is also a bad bet on its own terms, independent of the energy question**: `not_mhnhej` pins only `is_cut` (never `is_hdr`), so even at its own build seed (401) `consistency_factor` reaches only **0.2743** — nowhere near the 1.0000 that `hdr`/`mh` reach when used the same way — and averaged over the contract's 3 real seeds it falls to **0.1558**, near the ordinary dirty-seed floor (~0.10-0.19) despite every row being individually rule-compliant at the build seed. Reaching real energy variance would require deliberately selecting farther/lower-GC candidates exactly as `energy_hdr_k562.py`'s `spread` arm did — already measured to cost `total_weighted_score` in half and still land the wrong sign on K562 — so this is not a promising direction to re-open with a different sign hypothesis. **The 5-factor extension is PROVABLY unable to beat energy alone, and that is now confirmed at population scale rather than assumed from the formula.** `stage3.repair_mode(cas, energy, mh, rng)` takes only `cas`/`energy`/`mh` as causal inputs, and conditioning on `not_mhnhej` (excluding the `mh_nhej` draw segment from a uniform draw) leaves `P(HDR \| not_mhnhej) = hdr/(hdr+blunt)` — a ratio in which `mh_nhej`'s width, and therefore `mh`, cancels out EXACTLY (removing a middle segment from a uniform distribution rescales but does not change the relative proportions of the segments either side of it). `gc`/`distance`/`consistency`/`dist_score`/`gc_score` reach the outcome only THROUGH their deterministic contribution to `energy` (`stage3.sequence_energy`), so once `energy` is known they carry zero additional information for this question — a mathematical certainty from the code, not an empirical claim, but checked anyway per this file's own rule. **Checked on the full K562 candidate pool for one contract (193,135 `not_mhnhej`-compliant candidates across all 8 stage-5 cells, not the 240-row submission subsample)**: 5-fold cross-validated logistic regression AUC for energy alone is **0.5226 +/- 0.0015**, for all 5 features combined **0.5229 +/- 0.0016** — a gap of **+0.0003**, matching the near-zero prediction, against a closed-form theoretical ceiling of only **0.5271** (barely above chance, because K562's energy is heavily saturated: mean 0.9149, 57% of the pool sits at the 1.0 clamp). Model B's fitted coefficients confirm the mechanism directly rather than just the aggregate score: `distance` fits to **exactly 0.0000**, `gc`/`consistency` get small/wrong-signed coefficients (-0.0523/-0.0333), `mh` gets **+0.0009** (indistinguishable from the algebraic zero it should be), and `energy` absorbs essentially the whole signal (+1.0181, close to the energy-only model's own +0.9015). So there is no combination of these 5 factors, at any sample size, that can do better than energy alone on this question — the ceiling was already measured by the energy-only test below and this does not raise it. **The underlying energy-only finding, on both cells:** `stage3.repair_mode` gives `P(HDR \| not_mhnhej) = hdr_w/(hdr_w+0.35)` with `hdr_w = hdr_base + 0.35*energy`, a genuine, deterministic dependence (0.41-0.48 at energy=0 up to 0.63-0.66 at energy=1), and `energy` IS one of stage4's seven `build_X` features — so the dependence is provably real and in principle learnable. But on a real K562 contract at a real stamped seed, the natural (nearest-first) candidate pool saturates `energy` to **exactly 1.0000 with zero variance** for all 240 rows (matching the existing "energy is CLAMPED at 1.0 on every erythroid row" note), so `Pearson(energy, is_hdr) = 0` by mathematical necessity, not measurement. Deliberately forcing variance (half nearest/half farthest within `max_distance` 600, energy 0.7259-1.0000) did not help — `Pearson` came out **-0.0362** (wrong sign) and is_hdr's R2 got WORSE (-0.1528 against the zero-variance baseline's -0.1092), because the ACHIEVABLE energy range at that distance only implies a **~3.5-4.2 percentage-point** theoretical P(HDR) shift, smaller than the ~5.3-point sampling noise at n=80/tercile — the true signal exists but is swamped. This is not a sample-size problem (more seeds sharpen the estimate toward that same small shift, not reveal a bigger one); it is bounded by `stage3`'s own formula and by how little energy actually varies within `not_mhnhej`-compliant range on this cell. Widening `max_distance` well past 600 would reach genuinely lower-energy (low-GC, far) sites and open a larger theoretical range (~15-20 points), but `dist_score` decays with distance and the moderate widening already tried cut `total_weighted_score` 216 -> 112 — untested whether the wider range is worth that, but the economics look unfavourable given how weak the signal already is at the cheap end. **Re-run on HEK293 (2026-09-20, one contract, real stamped seed) and it still fails, for a DIFFERENT reason.** HEK293's low accessibility (0.35, against K562's 0.77) means `energy` never saturates: the whole 180,654-candidate pool spans **0.32-0.64 with ZERO at the 1.0 clamp** (against K562's ~57% clamped), so real variance exists even in the natural build (`default` arm: energy std 0.0078, not exactly 0 — narrow but nonzero). Forcing more of it (`spread`, energy std 0.15, range 0.32-0.64) finally gets the raw correlation to show the **theoretically-predicted positive sign** for the first time — `Pearson(energy, is_hdr)` **+0.1229**, and tercile means trend the right way (0.51 -> 0.60 -> 0.60) — confirming the mechanism is empirically visible on this cell where it structurally could not be on K562. **It still makes stage4's is_hdr R2 WORSE anyway** (-0.1004 on `default` -> **-0.5070** on `spread`), and `indel_length`'s R2 collapses alongside it (-0.2064 -> -0.4838) while `total_weighted_score` more than halves (270.9 -> 161.8): reaching for energy variance drags in farther, lower-GC candidates that vary on distance/gc/dist_score too, not just energy, and that extra heterogeneity gives the cross-validated RandomForest more to overfit on across folds rather than more signal to generalise from — a correlation visible in raw (non-CV) data does not survive 5-fold CV once the feature space gets messier. **`default` (plain nearest-first, no energy engineering) wins on `consistency_factor` on BOTH cells** (K562 0.3058, HEK293 0.2896, both the best of their three arms) — the failure mode differs (no achievable variance on K562; overfitting cost exceeding signal gain on HEK293) but the verdict does not: chasing this signal is dead on both cells tested. |
...
cut-only hedge work. Raising `max_fail`, `max_distance` or pool size does not move it. **`not_mhnhej`
was tested separately (2026-09-21, `sa_nomh_scratch.py`) and sits WORSE than this ceiling, not
inside it** — see the falsified-table row below for the full sweep; its per-guide failure rate is
too UNIFORM (no design lever spans more than ~3 percentage points) for the min-union greedy to find
anything to exploit, unlike whatever gives `hdr` its 349.
